...
class TestDVCPipeline:
    ...
    def test_all_stage_deps_exist(self, dvc_config):
        ...
        Deps that are outputs of earlier DVC stages (e.g. data/raw/) are
        skipped — they only exist after `dvc repro` runs those stages.
        """
        # Collect all outs across stages so we can skip generated deps
        all_outs = set()
        for stage in dvc_config["stages"].values():
            for out in stage.get("outs", []):
                all_outs.add(out.rstrip("/"))
                # Also add parent directories of outputs
                parts = Path(out).parts
                for i in range(1, len(parts)):
                    all_outs.add(str(Path(*parts[:i])))

        for name, stage in dvc_config["stages"].items():
            for dep in stage.get("deps", []):
                dep_clean = dep.rstrip("/")
                if dep_clean in all_outs:
                    continue  # Generated by an earlier stage
